...
def render_investment_timing_analysis():
    ...
    # 買い推奨条件の説明
    with st.expander("📋 買い推奨の判定条件"):
        st.markdown("""
        **買い推奨の条件:**
        - 🟢 **強い買い**: 戦略スコア80%以上 かつ データ信頼度80%以上
        - 🔵 **買い**: 戦略スコア70%以上 かつ データ信頼度60%以上
        
        **戦略別スコア算出基準:**
        - **ディフェンシブ**: 配当利回り(30点) + PER(25点) + PBR(25点) + ROE(20点)
        - **グロース**: ROE(40点) + 成長性指標(30点) + PER許容度(30点)  
        - **バリュー**: PER(40点) + PBR(40点) + 価格妥当性(20点)
        - **配当**: 配当利回り(50点) + 配当継続性(30点) + 財務安定性(20点)
        - **モメンタム**: 価格動向(50点) + 出来高(30点) + 市場センチメント(20点)
        
        **データ信頼度:**
        - 配当利回り・PER・PBR・ROEの4指標のうち取得可能な割合
        - 重大な警告がある場合は30%減点
        """)
